...
async def delete_form(form_id: int, db: Session = Depends(get_db)):
    try:
        ...
        response = {"message": f"Form with id {form_id} deleted successfully", "success": True}
        
        # Create Langfuse trace for form deletion
        try:
            trace = langfuse.trace(name="form_deleted")
            trace.update(
