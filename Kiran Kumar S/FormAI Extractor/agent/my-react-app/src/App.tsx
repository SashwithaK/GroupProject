...
// ---------- main component ----------
const App = () => {
  // Replaced single-file state with uploads array
  ...
  const [currentStep, setCurrentStep] = useState<number>(1); // 1: Upload/List, 3: Review
  ...
  const handleFilesSelect = (selectedFiles: File[]) => {
    ...
    selectedFiles.forEach(file => {
      if (!validTypes.includes(file.type)) {
        // setError("Skipped invalid file: " + file.name);
        return;
      }
      if (file.size > 10 * 1024 * 1024) {
        // setError("Skipped large file: " + file.name);
        return;
      }
    ...
    // Automatically switch to list view if we were in review mode
    if (currentStep === 3) setCurrentStep(1);

    // Trigger uploads
    newUploads.forEach(item => uploadToBackend(item));
  };
  ...
  const uploadToBackend = (item: UploadItem) => {
      ...
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status >= 200 && xhr.status < 300) {
          try {
            ...
            startPolling(item.id, resp.runId);
          } catch (e) {
            console.error(e);
            updateUpload(item.id, {
  ...
  const startPolling = (uploadId: string, runId: string) => {
    const poll = async () => {
      try {
        ...
        setUploads(prev => prev.map(u => {
          ...
          let newProgress = u.progress;
          if (newStatus === "QUEUED" && newProgress < 40) newProgress += 3;
          else if (newStatus === "PROCESSING" && newProgress < 90) newProgress += 5;
          else if (newStatus === "COMPLETED" || newStatus === "FAILED") newProgress = 100;

          return {
            ...u,
            status: newStatus,
            error: data.error ?? u.error,
            result: data.result ?? u.result,
            isProcessing: !(newStatus === "COMPLETED" || newStatus === "FAILED"),
  ...
  const handleViewResult = (item: UploadItem) => {
    if (item.result) {
      setExtractedData(item.result);
      setViewingUploadId(item.id);
      setCurrentStep(3);
    }
  };
  ...
  const downloadCSV = () => {
    const activeRunId = extractedData?.runId;
  ...
  const handleView = (runId: string) => {
    fetch(`${API_BASE}/extractions/${runId}`)
      ...
      .then(data => {
        ...
        // setRunId(runId); // No longer needed
        setCurrentStep(3);
  ...
  const retryProcessing = (runId: string) => {
    // Retry logic could be complex with multiple uploads, 
    // for now we can just re-upload the file if we had it, but history items don't store the File object.
    // So we'll just skip retry for history items for now or implement it later.
    console.log("Retry not fully implemented for history items yet");
  };
    ...
    <div className="space-y-8">
      ...
      <div className="text-center">
        <p className="inline-flex items-center gap-2 rounded-full bg-blue-50 px-4 py-1 text-xs font-medium text-blue-700 mb-4">
          ...
          <span>Step 1 · Upload handwritten forms</span>
        </p>
        <h2 className="text-3xl font-bold text-gray-900 mb-2">
          Upload Handwritten Forms
        </h2>
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          Drag and drop multiple files (images or PDFs). AI will process them in parallel.
        </p>
      </div>
      ...
      {uploads.length > 0 && (
        <div className="space-y-4">
          ...
          {uploads.map((item) => (
            <div key={item.id} className="bg-white rounded-xl p-4 shadow-sm border border-gray-200 flex items-center gap-4 transition-all hover:shadow-md">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${item.status === 'COMPLETED' ? 'bg-green-100' :
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-medium text-gray-900 truncate" title={item.file.name}>
                    {item.file.name}
                  </span>
                  <div className="flex items-center gap-3">
                    <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full tracking-wide uppercase ${item.status === 'COMPLETED' ? 'bg-green-100 text-green-700' :
                ...
                <div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
                  <div
                    className={`progress-bar-smooth h-full rounded-full ${item.status === 'COMPLETED' ? 'bg-green-500' :
                      item.status === 'FAILED' ? 'bg-red-500' :
                        'bg-gradient-to-r from-blue-500 to-purple-600 processing'
                      }`}
              ...
              {item.status === 'COMPLETED' && (
                <button
                  ...
                  className="px-4 py-2 text-sm font-medium bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 hover:border-gray-300 transition-colors shadow-sm flex items-center gap-2"
                >
            ...
            {filteredHistory.map((item) => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-[10px] ${item.status === "COMPLETED"
                ...
                {expandedRunId === item.runId && (
                  <div className="mt-3 pt-3 border-t border-gray-200 text-[10px] space-y-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className={`font-medium ${item.status === 'COMPLETED' ? 'text-emerald-600' :
