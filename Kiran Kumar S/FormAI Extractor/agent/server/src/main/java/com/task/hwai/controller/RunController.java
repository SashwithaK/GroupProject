...
public class RunController {
    ...
    public ResponseEntity<RunResponse> getRun(@PathVariable UUID runId) throws Exception {
        ...
        if (run.getStatus() == RunStatus.COMPLETED) {
            ...
            try {
                result = ex != null && ex.getResultJson() != null
                        ? JsonUtil.read(ex.getResultJson(), ExtractionResult.class)
                        : null;
            } catch (Exception e) {
                System.err.println("Error deserializing extraction result: " + e.getMessage());
                e.printStackTrace();
                return ResponseEntity.ok(
                        new RunResponse(RunStatus.COMPLETED, null, "Failed to deserialize result: " + e.getMessage()));
            }
            return ResponseEntity.ok(new RunResponse(RunStatus.COMPLETED, result, null));
        }
    ...
    public ResponseEntity<RunResponse> postRun(@PathVariable UUID runId) throws Exception {
        // Allow clients to POST to the same resource to trigger processing (returns
        // same shape as GET)
        var run = runRepo.findById(runId).orElse(null);
        if (run == null)
            return ResponseEntity.notFound().build();
    ...
    static class CsvUtil {
        ...
        static String escapeNode(com.fasterxml.jackson.databind.JsonNode n) {
            return escape(safeText(n));
        }
    }
}