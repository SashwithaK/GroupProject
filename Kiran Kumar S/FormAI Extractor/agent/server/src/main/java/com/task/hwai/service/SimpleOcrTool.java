...
public class SimpleOcrTool implements OcrTool {
    ...
    public List<PageResult> extract(Path path) {
        // Support configurable tessdata path via environment variable `TESSDATA_PATH`
        // or `TESSDATA_PREFIX`.
        ...
        if (tessdata == null || tessdata.isEmpty()) {
            ...
            if (java.nio.file.Files.exists(java.nio.file.Path.of("tessdata"))) {
                tessdata = java.nio.file.Path.of("tessdata").toAbsolutePath().toString();
            } else {
                tessdata = "/usr/share/tessdata"; // default for most Linux packages
            }
        }

        // Defensive check: ensure required traineddata file exists before invoking
        // native code.
        java.nio.file.Path engData = java.nio.file.Path.of(tessdata, "eng.traineddata");
        if (!java.nio.file.Files.exists(engData)) {
            String msg = String.format("Tesseract traineddata not found: %s. Set TESSDATA_PATH or mount tessdata.",
                    engData);
